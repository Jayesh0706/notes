...
Some very common `kubectl` commands (and what they do):
...
**A8.** A **Service** in Kubernetes is an abstraction that defines a logical set of Pods and a policy by which to access them. Services provide a stable network endpoint (IP address + port) for a group of Pods, enabling loose coupling between dependent components. Even if Pods come and go (get rescheduled, IPs change), the Service IP/port stays the same.
